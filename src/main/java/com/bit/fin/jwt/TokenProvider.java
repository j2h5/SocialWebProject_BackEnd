...
public class TokenProvider implements InitializingBean {  //bean 생성
    ...
    // Authentication객체의 권한정보를 이용해서 토큰을 생성하는 createToken메서드 추가
    ...
    //Token의 정보를 이용해 Authentication 객체를 리턴하는 메소드 생성
    public Authentication getAuthentication(String token) {

        //Token을 이용해 Claim을 만들고
        Claims claims = Jwts
                .parserBuilder()
                .setSigningKey(key)
                .build()
    ...
    //Token의 유효성 검사를 하는 메서드
    public boolean validateToken(String token) {
        try {
            //Token을 파라미터로 받아서 파싱을 해보기
            Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token);
            return true;
            // 위에서 나온 Exception들을 캐치하고 문제가 있으면 false, 정상이면 return true;
